...
public class MovingBlock : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //초기 위치
        defPos = transform.position;
        //프레임당 이동거리 = (전체이동거리 / 이동시간) * FixedDeltaTime
        perDX = (moveX / times) * Time.fixedDeltaTime;
        perDY = (moveY / times) * Time.fixedDeltaTime;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            //접촉한것이 플레이어라면 이동 블록의 자식으로 만들기 - 캐릭터가 올라타면 마찰을 줘서 이동이 같이 되게 해준다.
